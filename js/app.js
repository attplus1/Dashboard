...
(function () {
  ...
  async function loadSampleTrades(){
    try {
      const r = await fetch('data/sample_trades.csv', {cache:'no-store'});
      if (!r.ok) throw 0;
  ...
  async function init(){
    ...
    const [recon, bench, prices, momentum] = await Promise.all([
      loadSampleTrades(), loadJSON('data/benchmark.json'),
    ...
    if (recon){ setReconciliation(recon); }
      ...
      $('#kpi-grid').innerHTML = `<div class="notice" style="grid-column:1/-1">
        Could not load sample trades. Use <b>Import trades</b> to load your statement,
        or serve this site over HTTP (GitHub Pages) so <code>data/</code> can be fetched.</div>`;
    }
    applySourceNotes();
    renderAll();
